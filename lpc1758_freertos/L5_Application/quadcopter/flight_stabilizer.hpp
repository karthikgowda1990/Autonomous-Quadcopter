...
class FlightStabilizer : public MotorControllerIface
{
    ...
    protected:
         ...
         */
        void applyThrottleValues(void) ;
        /*TODO: Mayur: Changed from applyPropellerValues to applyThrottleValues*/
    ...
    private:
       ...
       // float calibration[3]; // to calibrate YAW axis pitch and roll not reqd
       // float oldyaw;
        bool mArmed;                ///< Flag if the quadcopter is armed
        uint32_t mLogFrequencyMs;   ///< The log frequency in milliseconds
        float             ypr[3];
        float pidPitchThrottle; /// tlm bhushan
        float pidRollThrottle; /// tlm bhushan
        float pidYawThrottle; // tlm bhushan
        float max,min;
