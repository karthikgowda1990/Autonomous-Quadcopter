...
#define SENSORS_RADS_TO_DPS               (57.2957795F)          /**< Degrees/s to rad/s multiplier */
...
void SensorSystem::convertRawAccelero(sensorData_t &data)
{
   // Uart2& u2=Uart2::getInstance();
    ...
    float acc_temp_x = data.converted.x * _lsm303Accel_MG_LSB * SENSORS_GRAVITY_STANDARD;
    float acc_temp_y = data.converted.y * _lsm303Accel_MG_LSB * SENSORS_GRAVITY_STANDARD;
    float acc_temp_z = data.converted.z * _lsm303Accel_MG_LSB * SENSORS_GRAVITY_STANDARD;

    u2.printf("%.2f %.2f %.2f\n", acc_temp_x, acc_temp_y, acc_temp_z);
#endif
...
#if 0
    float gy_temp_x = data.converted.x * SENSORS_RADS_TO_DPS;
    float gy_temp_y = data.converted.y * SENSORS_RADS_TO_DPS;
    float gy_temp_z = data.converted.z * SENSORS_RADS_TO_DPS;

    u2.printf("%.2f %.2f %.2f\n", gy_temp_x, gy_temp_y, gy_temp_z);
#endif
