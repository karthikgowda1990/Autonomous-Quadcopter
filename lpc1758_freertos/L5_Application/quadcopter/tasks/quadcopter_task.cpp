...
#define QUADCOPTER_SENSOR_FREQUENCY       (400)
 ...
 */
#define QUADCOPTER_ESC_UPDATE_FREQUENCY   (400)

extern float north_motor_value;
extern float south_motor_value;
extern float east_motor_value;
extern float west_motor_value;
...
bool quadcopterRegisterTelemetry(void)
{
    ...
    if (success) success = TLM_REG_DISK_QUAD_VAR("pid_aggr_pitch_kp", q.mPitchPid.mPidParams.aggrkp, tlm_float);
    if (success) success = TLM_REG_DISK_QUAD_VAR("pid_aggr_pitch_ki", q.mPitchPid.mPidParams.aggrki, tlm_float);
    if (success) success = TLM_REG_DISK_QUAD_VAR("pid_aggr_pitch_kd", q.mPitchPid.mPidParams.aggrkd, tlm_float);
    if (success) success = TLM_REG_DISK_QUAD_VAR("pitch_PIDmode", q.mPitchPid.activePIDtuningMode, tlm_int);
    ...
    if (success) success = TLM_REG_DISK_QUAD_VAR("pid_aggr_roll_kp", q.mRollPid.mPidParams.aggrkp, tlm_float);
    if (success) success = TLM_REG_DISK_QUAD_VAR("pid_aggr_roll_ki", q.mRollPid.mPidParams.aggrki, tlm_float);
    if (success) success = TLM_REG_DISK_QUAD_VAR("pid_aggr_roll_kd", q.mRollPid.mPidParams.aggrkd, tlm_float);
    if (success) success = TLM_REG_DISK_QUAD_VAR("roll_PIDmode", q.mRollPid.activePIDtuningMode, tlm_int);
    ...
    if (success) success = TLM_REG_DISK_QUAD_VAR("pid_aggr_yaw_kp", q.mYawPid.mPidParams.aggrkp, tlm_float);
    if (success) success = TLM_REG_DISK_QUAD_VAR("pid_aggr_yaw_ki", q.mYawPid.mPidParams.aggrki, tlm_float);
    if (success) success = TLM_REG_DISK_QUAD_VAR("pid_aggr_yaw_kd", q.mYawPid.mPidParams.aggrkd, tlm_float);
    if (success) success = TLM_REG_DISK_QUAD_VAR("yaw_PIDmode", q.mYawPid.activePIDtuningMode, tlm_int);
    ...
    if (FR_OK == (status_write = f_open(&fil, "1:input.txt",FA_OPEN_ALWAYS | FA_WRITE)))
{
    sprintf(data, ",%f/n", q.mCurrentAngles.pitch);
    f_lseek(&fil, f_size(&fil));
    status_write = f_write(&fil,data, sizeof(data),&bytesWritten);
    f_close(&fil);
    //timer_count = 1;
}
    ...
    if (success) success = TLM_REG_QUAD_VAR("pitch_throttle",   q.pidPitchThrottle, tlm_float);
    if (FR_OK == (status_write = f_open(&fil, "1:output.txt",FA_OPEN_ALWAYS | FA_WRITE)))
  {
      sprintf(data1, ",%f/n", q.pidPitchThrottle);
      f_lseek(&fil, f_size(&fil));
      status_write = f_write(&fil,data, sizeof(data),&bytesWritten);
      f_close(&fil);
      //timer_count = 1;
  }
    if (success) success = TLM_REG_QUAD_VAR("roll_throttle",   q.pidRollThrottle, tlm_float);
    if (success) success = TLM_REG_QUAD_VAR("yaw_throttle",   q.pidYawThrottle, tlm_float);

    if (success) success = TLM_REG_QUAD_VAR("north_motor_value", north_motor_value, tlm_float);
    if (success) success = TLM_REG_QUAD_VAR("south_motor_value", south_motor_value, tlm_float);
    if (success) success = TLM_REG_QUAD_VAR("east_motor_value",  east_motor_value, tlm_float);
    if (success) success = TLM_REG_QUAD_VAR("west_motor_value",  west_motor_value, tlm_float);
...
quadcopter_task::quadcopter_task(const uint8_t priority) :
   ...
   mSensorPollFrequencyMs(1000 / QUADCOPTER_SENSOR_FREQUENCY),  // TODO: for python app comment this and use the below one
   // mSensorPollFrequencyMs(1000 / 20),
...
bool quadcopter_task::init(void)
{
 ...
 /*   f.zeroIMU(); //added for auto calibration after sensors init!
    f.zeroYaw();
