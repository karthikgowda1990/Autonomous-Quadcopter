...
#define RC_RX_TASK_STACK_BYTES        (6 * 512)
...
int32_t rc_remote_task::getNormalizedValue(const uint32_t &pulseWidthUs, const uint16_t scalingFactor, const uint16_t subtractingFactor, bool throttle)
{
    ...
    int16_t minPRY = -30;//-45;
    int16_t maxPRY = 30;//45;
    //float scalingFactorPRY = 2.22;
    ...
    if (throttle) {
        ...
        //normalizedValue = normalizedValue/scalingFactorPRY; // pitch and roll to be -45 to 45
        if (normalizedValue < minPRY)
            normalizedValue = minPRY;
        if(normalizedValue > maxPRY)
            normalizedValue = maxPRY;
...
bool rc_remote_task::run(void *p)
{
    ...
    switch (channelData.channel)
    {
        ...
        case rc_chan4_throttle:
        {
            ...
            LD.setNumber(mRcRxFlightCmd.throttle);
        ...
        case rc_chan5:
        {
            uint8_t armedValue = (uint8_t) getNormalizedValue(channelData.pulse_time_us, armScalingFactor, armSubtractingFactor, true);
            mQuadcopter.setArmed( (armedValue >= armedThreshold) );
            break;
        }
/*
