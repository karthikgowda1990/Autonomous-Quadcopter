...
class PID
{
    ...
    public:
        ...
        /// Enumeration of the active PID tuning mode
         typedef enum {
             CONSERVATIVE = 0,
             AGGRESSIVE = 1
         } PID_tuning_mode_E;

        // Limit that determines tuning mode to be aggressive or conservative
         const float mAngleLowerLimit = 2;

        // The current active tuning mode
         PID_tuning_mode_E activePIDtuningMode = CONSERVATIVE;

        /**
          * Sets the PID tuning mode to be aggressive or conservative
          */
         void setPIDTuningMode(const float setpointValue, const float presentInputValue);
    ...
    public:
        ...
        const float aggressive_pid_scale;     ///< Scaling of the PID aggressive mode
    ...
    private:
        ...
        float mSampleTimeMs; ///< The sample time in milliseconds that we should process the PID loop
